...
unet_baseline.load_state_dict(torch.load('./model2_epoch_{epoch+1}.pth', weights_only=True, map_location=torch.device('cpu'))) 
unet_baseline.eval()

#zoe_depth = torch.load('backend/MO_full11l_model.pth', weights_only=False, map_location=torch.device('cpu'))
#zoe_depth.eval()
...
def predict_zoe_depth(address):
    ...
    common_transform = transforms.Compose([transforms.Resize((256,256)), transforms.ToTensor()])
    image = common_transform(image)
    image = image.unsqueeze(0)
